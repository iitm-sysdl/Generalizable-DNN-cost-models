...
def getharwareFeatures(net_dict):
    for key in net_dict:
        net_dict[key][2] = net_dict[key][2][:numLatency,:,:]
        net_dict[key][1] = net_dict[key][1][:numLatency]

    hw_features_cncat = []
    hardwareFeatures = np.genfromtxt('hardwareFeatures.csv', delimiter=',')
    files = glob.glob('*.txt')

    for key in net_dict:
        index = files.index(key)
        hw_features_per_device = list(hardwareFeatures[index])
        hw_features_cncat.append(hw_features_per_device)
...
def subsetAndLearn(net_dict, final_indices, numSamples):
    ...
    elif args.sampling_type == 'hardware':
        hw_features_cncat = getharwareFeatures(net_dict)
    else:
...
def learn_combined_models(list_val_dict):
    ...
    ######## Transfer for the remaining held-out set #############
    hw_features_cncat = getharwareFeatures(list_val_dict_30)
