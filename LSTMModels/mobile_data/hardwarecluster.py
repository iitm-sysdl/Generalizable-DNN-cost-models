...
def learn_combined_models(list_val_dict):
    ...
    fast = [3, 4, 6, 7, 11, 12, 15, 16, 17, 18, 20, 24, 26, 28, 31, 34, 35, 36, 37, 38, 39, 41, 45, 47, 49, 51, 55, 59, 60, 63, 66, 67, 68, 69, 70, 73, 77, 80, 81, 82, 86, 88, 89, 91, 94, 95, 97, 98, 103]
    medium = [0, 1, 2, 8, 10, 13, 14, 19, 21, 23, 25, 29, 30, 32, 33, 40, 43, 46, 53, 54, 56, 57, 61, 62, 64, 65, 71, 72, 76, 78, 79, 85, 87, 90, 93, 99, 101, 102, 104]
    slow = [5, 9, 22, 27, 42, 44, 48, 50, 52, 58, 74, 75, 83, 84, 92, 96, 100]
    print(len(slow), len(medium), len(fast))
    if args.transfer == 'slow':
        train = list(set().union(fast, medium))
        test = slow
    elif args.transfer == 'medium':
        train = list(set().union(fast, slow))
        test = medium
    elif args.transfer == 'fast':
        train = list(set().union(medium, slow))
        test = fast
    ...
    for key in list_val_dict:
        if i in train:
            list_val_dict_train[key] = list_val_dict[key]
        elif i in test:
            list_val_dict_test[key] = list_val_dict[key]
        i += 1
    print(len(list_val_dict), len(list_val_dict_train), len(list_val_dict_test))
    #exit(0)

    if args.sampling_type == 'random':
        hw_features_cncat = random_sampling(list_val_dict_train, final_indices, args.numSamples)
    elif args.sampling_type == 'statistical':
        final_indices, hw_features_cncat = sample_hwrepresentation(list_val_dict_train, args.numSamples)
    elif args.sampling_type == 'mutual_info_v1':
        final_indices, hw_features_cncat = mutual_information(list_val_dict_train, args.numSamples)
    elif args.sampling_type == 'mutual_info_v2':
        final_indices, hw_features_cncat = mutual_information_v2(list_val_dict_train, args.numSamples, choose_minimal=False)
    elif args.sampling_type == 'spearmanCorr':
        final_indices, hw_features_cncat = spearmanCorr(list_val_dict_train, args.numSamples)
    elif args.sampling_type == 'pearsonCorr':
        final_indices, hw_features_cncat = pearsonCorr(list_val_dict_train, args.numSamples)
    else:
        print("Invalid --sampling_type - Fix")
        exit(0)

    dumpSelectedNetworks(final_indices)
    final_lat, final_features = append_with_net_features(list_val_dict_train, hw_features_cncat)
    ...
    if args.model=='lstm':
        model, modellist, extractor = learn_lstm_model(hardware, list_val_dict[files[0]][0], final_lat, final_features, final_features.shape[2])
    elif args.model=='xgb':
        model = learn_xgb_model(hardware, list_val_dict[files[0]][0], final_lat, final_features, final_features.shape[2])

    for key in list_val_dict_test:
        list_val_dict_test[key][2] = list_val_dict_test[key][2][:numLatency,:,:]
        list_val_dict_test[key][1] = list_val_dict_test[key][1][:numLatency]

    hw_features_cncat = []
    for key in list_val_dict_test:
        hw_features_per_device = []
        for j in range(len(final_indices)):
            hw_features_per_device.append(list_val_dict_test[key][1][final_indices[j]])
        hw_features_cncat.append(hw_features_per_device)

    #If this is not done separately, the code will break
    for key in list_val_dict_test:
        list_val_dict_test[key][1] = np.delete(list_val_dict_test[key][1], final_indices, axis=0)
        list_val_dict_test[key][2] = np.delete(list_val_dict_test[key][2], final_indices, axis=0)


    final_lat_30, final_features_30 = append_with_net_features(list_val_dict_test, hw_features_cncat)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--transfer", type=str, help='To transfer', required=True)
