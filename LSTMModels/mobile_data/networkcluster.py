...
large = [35, 37, 40, 44, 45, 46, 48, 49, 50, 52, 53, 56, 57, 61, 63, 64, 65, 66, 70, 72, 76, 81, 83, 85, 87, 88, 89, 90, 91, 93, 94, 96, 97, 98, 99, 102, 103, 104, 105, 107, 110, 111, 114, 115]
giant = [33, 34, 36, 38, 39, 41, 42, 43, 47, 51, 54, 55, 58, 59, 60, 62, 67, 68, 69, 71, 73, 74, 75, 77, 78, 79, 80, 82, 84, 86, 92, 95, 106, 108, 112, 113]
...
def random_indices(maxSamples):
    rand_indices = []
    if args.networkcluster == "all":
        for i in range(maxSamples):
            rand_indices.append(random.randint(0,numLatency))
    elif args.networkcluster == "small":
        rand_indices = random.sample(small, maxSamples)
    elif args.networkcluster == "large":
        rand_indices = random.sample(large, maxSamples)
    elif args.networkcluster == "giant":
        rand_indices = random.sample(giant, maxSamples)    
    return rand_indices
'''
...
def mutual_information_v2(net_dict, numSamples, choose_minimal=True):
    ...
    if args.networkcluster == "small":
        stacked_arr = stacked_arr[small]
    elif args.networkcluster == "large":
        stacked_arr = stacked_arr[large]
    elif args.networkcluster == "giant":
        stacked_arr = stacked_arr[giant]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--networkcluster", type=str, help='Sample from small, large, giant,all')
