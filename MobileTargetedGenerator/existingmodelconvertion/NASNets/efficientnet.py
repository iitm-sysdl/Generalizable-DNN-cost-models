...
from .efficientnet_blocks import netEmbedding
...
class EfficientNet(nn.Module):
    ...
    def forward_features(self, x):
        ...
        inDim = ConvBNReLUEmbedding(x.shape[3], self.in_planes, self.stem, 3, 1, self.pad, 1, netEmbedding)
        x = self.conv_stem(x)
        x = self.bn1(x)
        x = self.act1(x)
        ...
        inDim = ConvBNReLUEmbedding(x.shape[3], self._in_chs, self.num_features, 1, 1, 1, 1, netEmbedding)
        x = self.conv_head(x)
        x = self.bn2(x)
        x = self.act2(x)
        return x
    ...
    def forward(self, x):
        ...
        pooling(x.shape[3], x.shape[3], x.shape[1], netEmbedding)

        ### What to do for this flatten?????
        x = x.flatten(1)
        if self.drop_rate > 0.:
            x = F.dropout(x, p=self.drop_rate, training=self.training)
        convolution(1, x.shape[1], 1000, 1, 1, 1, netEmbedding)
        ...
        data=data[:-1]
        data=data+'\n'
        ofile = open("EfficientNetB0.csv", "w")
        ofile.write(data)
        ofile.close()
