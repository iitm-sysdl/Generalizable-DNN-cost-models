...
def learn_xgb_model(hardware, maxLayer, lat_mean, features, featuresShape, splitPercentage=1, shuffleFeatures=True):
...
#   writeToFile("The R^2 Value for %s: %f"%(hardware, r2_score))
#   writeToFile("The Spearnman Coefficient and p-value for %s: %f and %f"%(hardware, s_coefficient, pvalue))
#   dumpTabledata('Base_Set', r2_score)
...
def learn_collaborative_models(list_val_dict):
    ...
    if args.model == "lstm":
        ...
        model, final_indices, final_lat, final_features = subsetAndLearn(list_val_dict_base_set, final_indices, args.numSamples)
    ...
    for key in list_val_dict_rem:
        print("Adding New Hardware: %s==========================" %(key))
        i += 1
        tempDict = {}
...
def checkTransfer(lat, features, model, final_indices, modellist = None, extractor = None, hardware="Mixed Model"):
    ...
    if args.model == 'lstm':
        ...
        r2_score = sklearn.metrics.r2_score(testy, testPredict)
        s_coefficient, pvalue = spearmanr(testy, testPredict)
        writeToFile("The transferred R^2 Value for %s is: %f"%(hardware, r2_score))
        writeToFile("The transferred Spearnman Coefficient and p-value for %s set is: %f and %f"%(hardware, s_coefficient, pvalue))
        ...
        return [r2_score, s_coefficient, a, b]


def learn_combined_models(list_val_dict, list_val_dict_gpu):
    final_indices = 0
    if args.sampling_type == 'random':
        final_indices = random_indices(args.numSamples)
    if args.model == "lstm":
    ...
    ######## Transfer for all hw #############
    i = 0
    for key in list_val_dict_gpu:
        tempDict = {} 
        tempDict[key] = list_val_dict_gpu[key]
        # print(tempDict[key][1].shape, tempDict[key][2].shape) 
        tempDict, hw_features_cncat = cncatHardwareRep(tempDict, final_indices)
...
def main():
    ...
    for file in gpufiles:
        latency = parse_latency(file)
        key = remove_prefix(file, 'mobileGPUdata/')
        gpukeys.append(key)
        tmp_list = []
        tmp_list.append(maxLayers)
        tmp_list.append(latency)
    ...
    if args.learning_type == 'individual':
        ...
        learn_combined_models(list_val_dict, list_val_dict_gpu)
